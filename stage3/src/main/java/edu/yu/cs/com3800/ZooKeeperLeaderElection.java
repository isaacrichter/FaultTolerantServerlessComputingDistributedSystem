...
public class ZooKeeperLeaderElection implements LoggingServer{
    ...
    public synchronized Vote lookForLeader() {
        try {
            this.logger = initializeLogging(this.myPeerServer.getServerId(), this.getClass().getCanonicalName());
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void processVote(ElectionNotification currentNotification){
        ...
        if (supersedesCurrentVote(currentNotification.getSenderID(), currentNotification.getPeerEpoch())) {
            this.logger.log(Level.INFO, "got a better vote-- changing vote to server: " + currentNotification.getProposedLeaderID());
        ...
        if(haveEnoughVotes(votes, currentNotification)){
            this.logger.log(Level.INFO,   currentNotification.getProposedLeaderID() + " seems to have enough votes...");
            //      first check if there are any new votes for a higher ranked possible leader before I declare a leader. //////use a a while loop
            try {
                this.logger.log(Level.INFO, "checking Q... ");
                Message n = null;
                while ((n = incomingMessages.poll(finalizeWait, TimeUnit.MILLISECONDS)) != null){
                    ...
                    this.logger.log(Level.INFO, "Q says: server " + en.getSenderID() + " votes for " + en.getProposedLeaderID());
                    ...
                    if(supersedesPresumedWinner(en.getProposedLeaderID(), en.getPeerEpoch(), currentNotification.getProposedLeaderID(), currentNotification.getPeerEpoch())){
                        ...
                        this.logger.log(Level.INFO, currentNotification.getProposedLeaderID() + " not accepted as winner - had incoming vote for " + en.getProposedLeaderID() + " by server " + en.getSenderID());
                        break;

                    }
                }
                if (n == null){
                    this.logger.log(Level.INFO, currentNotification.getProposedLeaderID() + " accepted as leader!" );
                    this.acceptElectionWinner(currentNotification);
                }
            }catch(Exception e){ this.logger.log(Level.INFO, "error occured");}
        }
    }
    ...
    protected boolean haveEnoughVotes(Map<Long, ElectionNotification> votes, Vote proposal) {
        ...
        this.logger.log(Level.INFO, "not enough votes. Needed " + majority + " for majority " + "only had " + voteCounter);
