...
public class Stage2Test {
    ...
    @Test
    public void numServerTest(){
        //highest allowed port number is 65,535
        setupAndTestServers(5, 1000);
        setupAndTestServers(15, 4000);
        setupAndTestServers(30, 8000);

    }

    public void setupAndTestServers(int numberOfServers, int port){
        //create IDs and addresses
        HashMap<Long, InetSocketAddress> peerIDtoAddress = new HashMap<>(3);
        for(long i = 0L; i < numberOfServers; i++){
            peerIDtoAddress.put(i, new InetSocketAddress("localhost", port));
            port += 10;
        }

        //create servers
        ArrayList<ZooKeeperPeerServer> servers = new ArrayList<>();
        for (Map.Entry<Long, InetSocketAddress> entry : peerIDtoAddress.entrySet()) {
            HashMap<Long, InetSocketAddress> map = (HashMap<Long, InetSocketAddress>) peerIDtoAddress.clone();
            map.remove(entry.getKey()); //why remove sent? might need to change
        ...
        }while((!finished) && ((System.currentTimeMillis() - startTime) < 120000)); // exit once all have found leader, or two minutes have passed
        ...
        for (ZooKeeperPeerServer server : servers) {
            ...
            if (leader != null) {
                ...
                System.out.println("Server on port " + server.getAddress().getPort() + " whose ID is " + server.getServerId() + " had a null leader and is " + server.getPeerState().name());
                server.shutdown();
