            ...
            while (!this.shutdown){
                switch (getPeerState()){
                    case LOOKING:
                        ...
                        ZooKeeperLeaderElection election = new ZooKeeperLeaderElection(this, incomingMessages, observers.size(), peerIDtoAddress.size());
                        election.lookForLeader(); // sets state, leader and clears incoming messages
                        election = null; // allows the now old one to get gc'ed
                        logger.log(Level.FINE, "Election ended");
                    ...
                    case FOLLOWING:
                        if (javaRunnerFollower == null){
                            ...
                            listener = new TCPListener(this.myPort, incomingTCPMessagesAndSockets, this.id);
