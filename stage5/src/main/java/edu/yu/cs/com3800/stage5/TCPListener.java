...
public class TCPListener extends Thread implements LoggingServer {
    ...
    public TCPListener(int serverPort, LinkedBlockingQueue<MessageAndSocket> tcpMessagesWithSockets, Long id, ZooKeeperPeerServerImpl server){
        ...
        while(!this.interrupted){
            try {
                ...
                if(server.isPeerDead(new InetSocketAddress(message.getSenderHost(), message.getSenderPort()))){
                    logger.fine("Message is from dead peer. Ignoring");
                }else {
