    ...
    class MyHandler implements HttpHandler {
        public void handle(HttpExchange t) throws IOException {
            ...
            if ((!headers.containsKey("Content-Type")) || !headers.get("Content-Type").get(0).equals("text/x-java-source")){
                ...
                InputStream reis;
                do {
                    while(presumedLeader == null) {
                        leaderAddress = gatewayPeer.getLeaderInetSocket();
                        presumedLeader = gatewayPeer.InetSocketAddressToServerID(leaderAddress);
                    }
                    ...
                    socket = new Socket(leaderAddress.getHostName(), leaderAddress.getPort() + 2); //tcp port is + 2 of the udp port
                    ...
                    workMessage.setRequestID(requestID);
                    ...
                    reis = socket.getInputStream();
                    while(reis.available() == 0 && !(gatewayPeer.getCurrentLeader() == null || presumedLeader != gatewayPeer.getCurrentLeader().getProposedLeaderID())){
                        try {
                            Thread.sleep(500);
                ...
                }while (gatewayPeer.getCurrentLeader() == null || presumedLeader != gatewayPeer.getCurrentLeader().getProposedLeaderID());

                byte[] responseBytes = Util.readAllBytes(reis);
                response = new Message(responseBytes);
                responseBodyBytes = response.getMessageContents();
