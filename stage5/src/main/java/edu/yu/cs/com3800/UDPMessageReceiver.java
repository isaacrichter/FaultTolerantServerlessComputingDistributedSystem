...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    public UDPMessageReceiver(LinkedBlockingQueue<Message> incomingMessages, InetSocketAddress myAddress, int myPort, ZooKeeperPeerServer peerServer, LinkedBlockingQueue<Message> gossipMessages) throws IOException {
        this.incomingMessages = incomingMessages;
        this.myAddress = myAddress;
        this.myPort = myPort;
        this.logger = initializeLogging(peerServer.getServerId(), "UDPMessageReceiver");
        this.setDaemon(true);
    ...
    public void run() {
        ...
        try {
            ...
            this.logger.log(Level.FINE, "failed to create receiving socket", e);
            return;
        }
        //loop
        while (!this.isInterrupted()) {
            try {
                ...
                if (sendLeader(received)) {
                    ...
                    //end stage 5 logic
                }else if(received.getMessageType() == Message.MessageType.GOSSIP ) {
                    gossipMessages.offer(received);
